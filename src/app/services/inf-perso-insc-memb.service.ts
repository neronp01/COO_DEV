...
membreUpdateInfPerso(prenom: string, nom: string , adresse: string , codePostal: string , ville: string ,
  ...
  const upMembre = {
    id: this.uid , infFacturation: membre.infFacturation , courriel : this.se.currentUserEmail , prenom : prenom , nom : nom ,
 ...
 get infoMembre(): Observable<Membre[]> {
  ...
  updateInfoMembre( dataMembre: Membre) {
    ...
    this.membreDoc = this.dbc.doc<Membre>('membres/' + dataMembre.id);
